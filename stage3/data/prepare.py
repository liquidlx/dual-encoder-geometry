...
def main():
    ...
    # Write tokenizer metadata -- CodeT5+ tokenizer needs no serialization
    ...
    print(f"\nCodeT5+ tokenizer sanity check:")
    print(f"  Original: {repr(sample)}")
    print(f"  Token ids (first 10): {[x for x in ids if x != tok.pad_token_id][:10]}")
    print(f"  Decoded:  {repr(decoded)}")
    print(f"  Lossless: {sample.strip() == decoded.strip()}")

    vocab_path = OUT_DIR / "vocab.json"
    tok.save(vocab_path)
    print(f"  Tokenizer: CodeT5+ ({tok.vocab_size} tokens) -> {vocab_path}")
    ...
    print("\nDone. Next:")
    print("  python3 train.py --model baseline --epochs 20 --batch_size 4")
    print("  python3 train.py --model dual     --epochs 20 --batch_size 4")
